...
| No SCORE entries | Step 6 must report `SCORE: <n>/100` literally (rules/orchestrator-protocol.md, Literal report lines) |
| No LOCALIZE/REPRODUCE/DRIFT_CHECK/BLAST_RADIUS entries | Session predates the 2026-07-15 report-line mandate, or the lines were not literal (rules/orchestrator-protocol.md, Literal report lines); ast-grep usage alone never emits BLAST_RADIUS |
