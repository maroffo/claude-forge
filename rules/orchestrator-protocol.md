...
- Review agents are read-only **with respect to the main tree**: every launch carries `isolation: "worktree"`, so the writes empirical review needs (executable probes, mutation runs) land in the agent's own copy and never in the tree being built. Isolation, not permission: it bounds contamination, it does not prevent prompt injection. software-engineer is read-write, scoped to its assigned files.
