...
| `reviewer-isolation-guard.sh` | PreToolUse Agent | Denies a `*-reviewer` subagent launched without `isolation: "worktree"`: review agents write mutants and probe files, so an un-isolated launch lands those writes in the real tree. Only `worktree` satisfies it, `remote` included: a remote launch carries no isolation assertion in its brief either, so the reviewer self-downgrades and the review silently degrades. The deny reason carries both remedies, relaunch with the parameter or make `ISOLATION-EXEMPT: <reason>` the FIRST line of the prompt, the only place the marker is honoured, so that quoted untrusted content further down cannot exempt its own launch (and the marker silences the hook only, the reviewer stays read-only agent-side). Reviewer-ness is matched on a key folded to bare alphanumerics, with a `reviewer` suffix rather than `-reviewer`, because the tool's resolver folds case and separators alike (`Security-Reviewer`, `security_reviewer`, `security reviewer` all start the real one); a `subagent_type` carrying non-ASCII bytes is treated as undecidable and denied too, since the resolver also folds Unicode compatibility spellings that no byte-level match can follow. Fails OPEN on missing `jq`, an unparseable payload or a cwd outside a work tree, because a false DENY bricks every review in every session until someone notices. Blind to Workflow-tool `agent()` launches, which never reach a PreToolUse matcher |
