...
- [x] 2026-07-28 16:10 — Follow-ups agent:ready (richiesta Max): plan-template W(last)+DoD row 7 e issue-loop step 3b filano i follow-up con label di triage (`agent:ready` se il rubric regge, `needs-spec`/`human` altrimenti), mai unlabeled. Contract C5 `2026-07-28_followups-agent-ready.md`.
- [x] 2026-07-28 16:20 — Merge (autorizzati): wasit PR #451 -> dev (7 check verdi), forge PR #111 -> main, hikma-skills feat/dod-evidence-gate -> main. Fasi 1-2 in produzione.
- [x] 2026-07-28 16:50 — Fase 3: skill `bug-hunter-hikma` scritta, fast-path `agent:hunted` in issue-triage (era invisibile: lo scan droppa tutte le `agent:*`), label creata su wasit, contract C4 (forge PR #112 -> main), skill mergiata su hikma-skills main.
- [x] 2026-07-28 17:05 — Fase 3 RUN SUPERVISIONATO (exit criterion): 35 min su wasit, 10 ipotesi testate, **0 bug e2e-confermati** (clean report esplicito) + 1 finding statico reale verificato a mano: bulk findings endpoints gated su `findings:read` mentre mutano -> org_viewer può cambiare status/assignee (wasit#452, `agent:human`, no repro claim). Skill aggiornata con static-finding path + blind spots. Dedup non esercitato (nessuna issue `agent:hunted` esiste ancora): gap noto.
Next action: Fase 4 (trigger notturno) — bloccata sulla crontab install pending di Max; oppure secondo run del hunter su un'area diversa

## Surprises & Discoveries (fase 2-3)
- **L'e2e di wasit ha RBAC fail-open**: l'harness monta i moduli con `Enforcer: nil` e `authz.RBAC` è no-op su nil (`internal/authz/middleware.go:26-29`). Conseguenza pesante oltre al hunter: il junit e2e nel bundle non prova NIENTE sull'autorizzazione, e `TestEveryRBACCallsiteHasExplicitPolicy` verifica che esista *una* policy per callsite, non che l'azione combaci con l'effetto dell'handler. È esattamente il buco da cui è passato wasit#452.
- **Il primo run del hunter non ha trovato bug via test ma ne ha trovato uno vero leggendo spec-vs-codice.** Lezione incorporata nella skill: percorso statico separato (max 1/run, `agent:human`, mai `agent:hunted`, nota esplicita sul perché manca il repro), più la lista dei blind spot per non bruciare wall-clock dove il test non può arrivare.
- Il review round della fase 2 ha trovato che le celle Command del DoD sono raggiungibili da testo di issue GitHub via plan-forge, e che dod_run gira **fuori** dal permission layer di Claude Code: da lì l'allowlist di prefissi. È il tipo di superficie che nasce solo quando si automatizza l'esecuzione.
