...
| 14 | E2E row 7 (truncated -> converged=no) is unenforceable as designed | Matrix row downgraded to prose-only; the artifact literal carries no truncation term | A truncated agent counts as returned, so the hook cannot tell a truncation from a clean round. Adding `truncated=<n>` to the literal is a real option but it is a fourth field on a line three consumers already parse: deferred to a follow-up rather than bolted on mid-round |
| 15 | Enforcement point: Stop vs PreToolUse | **Open, escalated to Max**: the Stop gate ships as built; the preventing gate does not | The architecture reviewer is right that a Stop hook fires after the rounds are already spent, so it cannot prevent the cost the plan exists to cut, only refuse to bless it. The preventing point is a PreToolUse hook on reviewer Agent launches, counting waves since the last human turn. That is a different change with its own contract, and the plan locked "Stop hook" in decision 2: switching it mid-execution would be relitigating a locked decision without the human who locked it |
