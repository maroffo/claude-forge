...
- 2026-07-07: Workstream A (Serena) CLOSED, not adopted. Native first-party LSP plugins now cover live symbol navigation for the whole stack (`gopls-lsp`/`swift-lsp` enabled; `pyright-lsp`/`typescript-lsp` enabled 2026-07-07; `ruby-lsp` skipped, Ruby no longer in use) at zero MCP context cost. Serena registration was never present on this workstation. Contract 2026-07-05_serena-adoption Result row filed, verdict reverted. Only Workstream B (codemap-ephemeral) remains open for retrospective.
