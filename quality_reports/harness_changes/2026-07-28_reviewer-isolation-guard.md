...
- Adjusting the matcher may only ever WIDEN the deny set, never narrow it. The hook matches a normalised key, and the tool's resolver folds more spellings than any byte-level normalisation can follow (case, whitespace, Unicode compatibility forms), so a spelling the matcher cannot decide is denied rather than allowed. Every registered agent type is ASCII, so the conservative reading costs no legitimate launch; a change that lets a previously denied spelling through is a regression even when it looks like a false-positive fix.
