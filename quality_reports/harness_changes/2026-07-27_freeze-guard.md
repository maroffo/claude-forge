...
# Harness Change Contract: repo-local freeze boundary for Edit/Write/NotebookEdit/MultiEdit

## Component

- `hooks/freeze-guard.sh` (new): PreToolUse hook on Edit, Write, NotebookEdit, MultiEdit.
...
- `hooks/settings.example.json`: four PreToolUse matcher entries (Edit, Write, NotebookEdit, MultiEdit), timeout 10.
...
In sessions where `/freeze` is active, out-of-boundary Edit/Write/NotebookEdit/MultiEdit calls drop to zero
...
2. A traced session with a boundary set in which **more than zero out-of-boundary edits pass**
   through Edit/Write/NotebookEdit/MultiEdit. That means the resolution logic does not hold and the hook is
...
`~/.claude/hooks/_freeze_boundary.sh` symlinks and the four PreToolUse entries from
