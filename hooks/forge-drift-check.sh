...
scan_entries() {
  ...
  for _entry in "$_dir"/$_glob; do
    ...
    if [ -L "$_entry" ] && [ -e "$_entry" ]; then
      ...
      if [ "$_entry" -ef "$FORGE_ROOT/$_cat/$_name" ]; then
        _managed=1
      ...
      finding "dangling symlink $_entry -> $_target; fix: ln -sfn $FORGE_ROOT/$_cat/$_name $_entry"
    elif [ -f "$_entry" ]; then
      ...
      [ -f "$_src" ] || continue
      cmp -s "$_entry" "$_src" || finding "stale copy $_entry differs from $_src; fix: ln -sfn $_src $_entry"
    fi
  done
